...
export default function Suppliers({ searchQuery }) {
  ...
  useEffect(() => {
    setCurrentPage(1); // Reset to first page whenever user searches
  ...
  // --- FILTER & SEARCH LOGIC (Includes Global & Local Search) ---
  const filteredVendors = vendors.filter(v => {
    ...
    const matchesSearch = 
        ...
        v.id.toString().includes(finalSearch) ||
      ...
      {view === "list" ? (
                    ...
                    <tr key={v.id} className="hover:bg-slate-50/50 transition-colors group">
                      <td className="p-5 flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-slate-400 uppercase font-black tracking-tighter">Ref: #{v.id}</p>
                        </div>
                      </td>
                      <td className="p-5">
                      ...
                      <td className="p-5 text-center">
                        <div className="flex justify-center gap-2">
                          ...
                          <button onClick={() => handleDeleteVendor(v.id)} className="p-2.5 bg-rose-50 text-rose-500 rounded-xl hover:bg-rose-500 hover:text-white transition-all shadow-sm"><Trash2 size={14}/></button>
                        </div>
                      </td>
                    </tr>
                  ))}
        ...
        <div className="max-w-5xl mx-auto p-6 animate-in fade-in duration-500">
          <div className="flex items-center justify-between mb-8">
            ...
            <h1 className="text-2xl font-black text-slate-800 tracking-tight uppercase tracking-widest">{formData.id ? "Sync Profile" : "Registry Protocol"}</h1>
          </div>
          <SupplierForm 
            formData={formData} 
